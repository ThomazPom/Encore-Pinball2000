...
"════════════════════════════════════════════════════════════════════════\n"
"\n"
"  --lpt-device PATH | 0xBASE\n"
"                         Forward all guest LPT (0x378/0x379/0x37A) traffic\n"
"                         to a real parallel port. Two backends, selected\n"
"                         by argument format:\n"
"\n"
"                         PATH (e.g. /dev/parport0)  → ppdev backend.\n"
"                             Unprivileged (needs `lp` group). Default:\n"
"                             /dev/parport0, silent fallback to emulation\n"
"                             if absent.\n"
"\n"
"                         0xBASE (e.g. 0x378) or decimal → raw backend.\n"
"                             Direct ioperm()+inb/outb. Lower per-access\n"
"                             latency, sub-µs strobe pacing via outb(0x80).\n"
"                             Requires CAP_SYS_RAWIO (run as root, or one-time:\n"
"                             `sudo setcap cap_sys_rawio+ep ./build/encore`).\n"
"\n"
"                         PATH = 'none' / 'emu' → force emulation.\n"
"                         Explicit value given → fail hard if open fails\n"
...
"\n"
"      Example — drive a connected cabinet via ppdev (default backend):\n"
"          ./build/encore --game swe1 --lpt-device /dev/parport0\n"
"\n"
"      Example — drive via raw I/O (faster, requires root or setcap):\n"
...
"                           io-handled  (default, MAIN INVESTIGATED PATH) —\n"
"                              no CPU patch; the game runs its native PCI\n"
"                              probe, the BT-107 scribble is held at 0xFFFF\n"
"                              until xinu_ready then flipped to 0x0000, and\n"
"                              Encore answers DCS2 UART ports (0x138-0x13F)\n"
"                              in io.c. STATUS: UART RESET handshake (cmds\n"
"                              0x5800/0x5A00) answers correctly, but the\n"
"                              post-reset cmd stream (0x3A dong / 0xAA audio\n"
"                              init / mixer) has not yet been observed —\n"
"                              boot dong only, no in-game audio. Under\n"
"                              active investigation.\n"
"                           bar4-patch — TEMPORARY FALLBACK for working\n"
"                              audio. Pattern-scans for the DCS probe's\n"
"                              5-byte CMP/JNE prologue at xinu_ready and\n"
"                              byte-patches it to force dcs_mode=1, routing\n"
"                              audio through PCI BAR4 + sound.c. Plays full\n"
"                              audio (dong + init + mixer + samples) on\n"
"                              bundles whose probe matches the prologue\n"
"                              (most shipped builds). Fails on bundles\n"
"                              where the prologue is absent (SWE1 v1.3,\n"

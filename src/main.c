    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--game") == 0 && i + 1 < argc) {
            ...
            printf("Usage: encore [OPTIONS]\n\n"
                   ...
                   "\n"
                   "Emulator hot-keys (F-row, layout-independent):\n"
                   "  F1 / Esc       Quit\n"
                   "  F2             Flip display vertically\n"
                   "  F3             Screenshot\n"
                   "  F4             Toggle COIN DOOR (closed/open interlock)\n"
                   "  F5             Pulse 'Enter' button\n"
                   "                    (= Begin Test in attract,\n"
                   "                       Enter/Select while in service menus)\n"
                   "  F6             (no-op — bit unused on SWE1)\n"
                   "  F7 / F8        Host audio volume +/- (SDL mixer)\n"
                   "  F9             Pulse credits (= '5' key)\n"
                   "  F10            Pulse left action (= start surrogate)\n"
                   "  F11            Toggle LPT trace log\n"
                   "  F12            Dump guest switch state to stderr\n"
                   "\n"
                   "Coin-door panel — 4 physical buttons, dual-function by mode:\n"
                   "                       attract        |   service / test\n"
                   "                       --------------- + -----------------\n"
                   "  5  KP_5  LEFT     btn1: Service Credits | Escape  (Phys[9].b0)\n"
                   "  -  KP_-  DOWN     btn2: Volume −        | Menu Down (b1)\n"
                   "  =  KP_+  UP       btn3: Volume +        | Menu Up   (b2)\n"
                   "  ENTER KP_ENTER\n"
                   "          RIGHT    btn4: Begin Test       | Enter/Select (b3)\n"
                   "\n"
                   "Playfield (Phys[10] — bottom-row modifiers, AZERTY/QWERTY safe):\n"
                   "  LSHIFT          Left  flipper  (b5)\n"
                   "  LCTRL  / Z      Left  action   (b7)\n"
                   "  RCTRL  / .      Right action   (b6)\n"
                   "  RSHIFT          Right flipper  (b4)\n"
                   "  SPACE           Both actions (dismisses copyright screens)\n");
            exit(0);
        }
    }
}

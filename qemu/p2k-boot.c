 ...
 *
 * option ROM (first 32 KiB of bank0) is staged at physical 0x80000 and the
 * CPU is reprogrammed to enter 32-bit protected mode at CS:EIP = 0x08:0x801D9
 * with ESP = 0x8B000 and IF=0.
 *
 ...
 *
 *   - 0x1000 hits the same class of bug at 0x1008 in swe1-default cold
 *     boot. Earlier boot experiments masked the failure with an IDT[6]
 *     catchall stub silently swallowed the resulting #UD. Once we replaced
 *     that stub with proper TCG support for Cyrix 0F 3C only (and deleted
 *     the catchall), the latent wild-jump-into-GDT bug surfaced as a real
 *     Trap-to-monitor.
 *
...
{
    /* GDT layout (matches the prior reference):
     *   [0] Null
     *   [1] CS=0x08: type 0x9F (code+conforming+readable+accessed)
     *   [2] DS=0x10: type 0x93 (data+writable+accessed)
...
void p2k_post_reset(void *opaque)
{
    ...
    /* 1a. (BT-131 NIC LAN ROM is now an MMIO shadow installed at machine
     *     init time; no per-reset poke required.) */
    ...
    CPU_FOREACH(cs) {
        ...
        env->cr[0] |= CR0_PE_MASK | CR0_ET_MASK;  /* PE=1 + ET=1 (387 present) */

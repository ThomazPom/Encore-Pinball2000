 ...
 * stepped by 4 bytes (so the linear bank is 16 MiB).  The legacy
 * interleave_file() handler writes each chip pair at base + chip*2, then
 * steps +4.
 *
 ...
 * if either chip is missing, leave the bank pointer NULL. */

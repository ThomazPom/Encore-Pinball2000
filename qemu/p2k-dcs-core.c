...
void (*p2k_dcs_core_audio_execute_mixer)(uint16_t cmd,
                                         uint16_t data1,
                                         uint16_t data2) = NULL;
void (*p2k_dcs_core_audio_raw_cmd)(uint16_t cmd) = NULL;
 ...
 *
 * IMPORTANT — what these modes really mean: whether DCS commands flow
 * through the legacy UART at 0x138-0x13F vs BAR4 MMIO vs some third path.
 *
...
{
    s_core.cnt_flag++;
    if (p2k_dcs_core_audio_raw_cmd) {
        return p2k_dcs_adsp_flag_byte();
    }
...
{
    if (p2k_dcs_core_audio_raw_cmd) {
        return p2k_dcs_adsp_read_response();
    }
    if (s_core.count == 0) {
        return 0;
    }
...
void p2k_dcs_core_write_cmd(uint16_t cmd)
{
    s_core.cnt_cmd++;
    if (p2k_dcs_core_audio_raw_cmd) {
        p2k_dcs_core_audio_raw_cmd(cmd);
        return;
    }

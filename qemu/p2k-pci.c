 ...
 *
 *   - The "lie" is small and well bounded: three static vendor/device
 *     entries (MediaGX host bridge, WMS PRISM/PLX, Cx5520 ISA bridge)
 *     answered out of a tiny lookup table. PRISM's PCI scan finishes,
 *     finds the IDs it expects, then talks to the BARs we already
 *     map elsewhere — no QEMU PCI plumbing is wired up at all here.
 *   - There is no runtime overhead beyond a handful of `inl/outl`
 *     dispatches per cold boot.
 *   - The proper replacement is a full PCI host bridge plus real
 *     `PCIDevice` subclasses for PLX9054, MediaGX video, CS5530, and
 *     PRISM/PLX9050. That is multi-week refactor work touching every
 *     BAR consumer (`p2k-plx9054.c`, `p2k-gx.c`, `p2k-bar3-flash.c`,
 *     `p2k-dcs.c`) and produces **no observable runtime gain** for
 *     emulator-only boots until every one of those files is also
 *     ported to QOM/PCIDevice.
 *   - Until we need real PCI features (MSI, hotplug, hostdev pass-
 *     through, IRQ routing) that effort is unpaid for. The current
 *     stub is the cheapest correct emulation: it makes PRISM happy,
 *     stays out of the way of everything else, and is one file you
 *     delete the day a real host bridge lands.
 *
 ...
 *
 * Authoritative source: the prior reference (verified working on the
 * full boot path that booted to attract mode + sound).
 *
 * Bus 0:
...
/* PLX BAR addresses from the legacy reference, kept here so this file is

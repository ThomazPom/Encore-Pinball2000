 ...
 *
 * Narrow-route policy: DCS commands and responses are filtered through
 * the core, but UART-specific register traffic (IER, LSR, MCR, etc.) stays
 * local.
                               ...
                               uint64_t val, unsigned size)
{
     ...
     * emitting 16-bit DCS commands as TWO outb to 0x13c (HIGH then LOW),
     * so we assemble the pair unconditionally. The pair handler does NOT

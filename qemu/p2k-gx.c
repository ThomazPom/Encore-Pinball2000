 ...
 *                              to physical RAM at 0x800000, and the display
 *                              reads from that same RAM address.  GP BLT
 *                              engine and the display path both go through
 *                              0x800000 — keeping a single backing store
 *                              avoids divergence.
...
void p2k_install_gx_stub(void)
{
    ...
    *(uint32_t *)((uint8_t *)host + GX_BC_DRAM_TOP_OFF) = GX_BC_DRAM_TOP_VAL;
     ...
     * blits and the display read this same backing store: the FB window is
     * a mirror of physical RAM 0x800000.  Requires -m at least 16 MiB so
     * 0x800000+0x400000 fits. */

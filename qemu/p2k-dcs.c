 ...
 *
 * BAR4 access shape (DCS-2 protocol over PLX9054 BAR4 window):
                          ...
                          unsigned size)
{
    ...
    if (off == 0) {
        ...
        /* DCS command word.  In both io-handled and bar4-patch modes, BAR4
         * carries DCS commands (`dcs_mode==1` either way).  The mode label
         * only controls whether the CPU probe is .text-patched (we don't
         * patch in either mode; our BAR4 device responds correctly to the
         * natural probe). */
        p2k_dcs_core_note_source("BAR4");
        p2k_dcs_core_write_cmd(val & 0xFFFFu);
        return;
    }
    /* off == 2 (flag byte): the game stashes a value here that the next
     * read will reflect back. Forwarding to the core makes BAR4 look alive
     * on the boot write-then-read DCS detect, keeping the game on the BAR4

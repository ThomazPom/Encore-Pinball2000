...
> [!CAUTION]
> This is NOT a service guide. Always consult the original Williams Pinball 2000 Schematic Manual (16-10882) and Safety Manual (16-10878) for electrical specifications, fuse charts, and high-voltage safety procedures before working on cabinet hardware.
...
> [!NOTE]
> The MediaGX is a system-on-chip design from the 1990s. Graphics, memory controller, and PCI bridge are integrated on-die, which is why Encore must model the MediaGX-specific MMIO region at GX_BASE rather than emulating a standard PC graphics card.
...
> [!TIP]
> The PRISM card's PCI identity (vendor 0x146E, device 0x0001, class 0x030000 display) is essential for XINA boot. The game's PCI probe searches for this exact signature to locate the ROM banks and DCS interface.
...
> [!IMPORTANT]
> The 15 kHz horizontal scan rate (CGA-class) is incompatible with standard PC monitors. Real cabinets use arcade CRTs; Encore row-doubles the 640×240 framebuffer to 640×480 for compatibility with modern displays.
...
> [!TIP]
> The parallel port serves triple duty: switch matrix input, lamp matrix output, and coil driver output. The switch-column strobe (register 0x05) also refreshes the blanking/watchdog circuit — if strobes stop for ~2.5 ms, all power drivers disable. See [37-power-driver-board-protocol.md](37-power-driver-board-protocol.md).
...
> [!WARNING]
> Bulging/leaking motherboard capacitors (typically 1000 µF / 6.3 V) are the most common P2K hardware failure. Replace proactively with 10 V low-ESR parts before they damage PCB traces or cause boot failures.

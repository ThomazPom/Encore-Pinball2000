# 39 — Community updates (mypinballs.com / Jim Askey)
...
> [!IMPORTANT]
> Community updates from mypinballs.com (Jim Askey) are fully supported by Encore and included in regression testing. We do not redistribute them — download directly from <https://mypinballs.com> to get the latest versions and support Jim's ongoing maintenance work.
...
|------|--------:|---------------------------------------|
| SWE1 | v2.00   | `pin2000_50069_0200_*_B_10000000`     |
| SWE1 | v2.01   | `pin2000_50069_0201_*_B_10000000`     |
...
> [!NOTE]
> Version number resolution is flexible: `--update 2.1`, `--update 210`, and `--update 0210` all resolve to the same directory. The `vvvv` field in directory names follows the convention `major*100 + minor*10`.
...
* **DCS sound** works on community updates the same way it does on
  ...
  [README.md](README.md) for DCS mode documentation.
...
* **Symbol tables** are stripped on community v2.x bundles in the
  ...
  [29-tools-sym-dump.md](README.md).

> [!TIP]
> Savedata is shared across versions of the same game. Switching from official Williams v1.8 to community v2.6 preserves high scores and adjustments. If you hit a checksum error, delete the `.nvram` file and let the game rebuild it.
...
* **Attract-screen wall-clock (community v2.1).** Both community v2.1
  ...
  bug. See known-limitations documentation for
...
> [!NOTE]
> The community v2.1 attract-screen wall-clock draws time/date from CMOS but uses different register interpretations than Williams' original code. Encore feeds the CMOS exactly as the original BIOS did; clock mismatches are a firmware quirk, not an emulator bug.

# 29 — Real-Cabinet Testing — Call to Action

> **You can help make Encore better.** Encore has been written and
> verified entirely in software, on a modern Linux host. The next big unknown is whether the same binary
...
* Whether the Debian 13 build instructions in
  ...
  [README.md](README.md) actually work end-to-end on
...
> [!NOTE]
> The primary goal is desktop validation on standard Linux distributions, not immediate cabinet integration. Community feedback on build success, boot timing, attract-mode fidelity, and basic controls helps prioritize the roadmap.
...
* **LPT passthrough on a bench driver board.** Connect a spare driver
  ...
  events. See [46-real-lpt-passthrough.md](46-real-lpt-passthrough.md)
...
> [!IMPORTANT]
> Real-hardware tests (LPT passthrough, NVRAM persistence, DCS audio, full game loops) are bonus validation. These tests require access to P2K cabinets or spare driver boards and should only be attempted after desktop-level validation succeeds.
...
> [!TIP]
> A terse report following the template above is more valuable than detailed speculation. We need empirical data points: did it build? did it boot? how long to attract? does audio play? Focus on observable facts, not root-cause analysis.
...
shipping bundle (see the testing documentation).
...
index `0x05`), see
[37-power-driver-board-protocol.md](37-power-driver-board-protocol.md). That page

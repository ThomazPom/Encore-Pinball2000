# 37 — Power Driver Board LPT Protocol References
...
(see [46-real-lpt-passthrough.md](46-real-lpt-passthrough.md)) can be aligned with
what the hardware actually expects.

> [!WARNING]
> The current emulated decoder and the raw passthrough were built without access to a real driver board. They are expected to need revision once tested on hardware. Treat everything below as the documented target, not as a description of Encore's current behavior.

The current emulated decoder in the QEMU machine and the raw passthrough were built without access to a real driver board. They are
...
> [!NOTE]
> PinballDiag's watchdog-keepalive thread exists because PinballDiag is a standalone diagnostic with no game code issuing switch-column strobes. When a P2K game is running, the natural switch-matrix scan **is** the keepalive.
...
Encore's raw backend now matches this.
...
> [!TIP]
> The PC parallel port inverts Strobe (bit 0) and Select (bit 3) in hardware. The driver board sees the inverted signals, so logical 1 in software becomes physical 0 on the wire. PinballDiag uses a shadow control byte to manage this transparently.
...
> [!IMPORTANT]
> Register 0x05 (Switch Column) serves double duty: it strobes the switch-matrix columns **and** refreshes the blanking/watchdog circuit. If column strobes stop for ~2.5 ms, blanking asserts and all power I/O drivers disable (lamps, solenoids, flashers all stop). This is the critical timing constraint for LPT passthrough.
...
> [!CAUTION]
> The 2.5 ms blanking window is the tightest real-time constraint in the entire system. On a real cabinet, if the guest's switch-column strobes (register 0x05) do not reach the wire within this window, all power drivers shut down immediately. This constraint makes LPT passthrough latency-sensitive.
...
- The raw backend forwards the guest's CTL byte (LPT
  ...
  real-LPT handler is a naked `outb`, with no CTL rewriting and
...
> [!TIP]
> Use `-vv` logging with real hardware to measure the actual time delta between guest 0x05 strobes. This empirical data will reveal whether host-side latency is the bottleneck or whether the game's natural scan rate keeps blanking cleared.

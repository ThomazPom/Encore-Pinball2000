...
| `--lpt-input` | `physical` \| `hybrid` | `physical` | Select physical-only cabinet input or the experimental additive keyboard overlay on active-low physical input reads. Hybrid closes requested switch bits without reopening physical closures and never modifies outputs; F4 remains physical-only. | `scripts/run-qemu.sh --lpt-device /dev/parport0 --lpt-input hybrid` |

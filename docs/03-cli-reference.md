...
| `--dcs-engine` | `pb2kslib` \| `pb2kslib-adsp` \| `adsp` \| `adsp-thread` | `adsp-thread` | Selects extracted samples, persistent PCM generated by the native DSP, synchronous native ADSP, or the condition-driven native ADSP worker. Missing native assets fall back to `pb2kslib`. | `scripts/run-qemu.sh --dcs-engine pb2kslib-adsp` |
| `--dcs-pcm-cpu` | logical CPU | unset | Pins only the `adsp-thread` PCM/mailbox worker. Experimental and Linux-only; validate on the target host. | `scripts/run-qemu.sh --dcs-engine adsp-thread --dcs-pcm-cpu 2` |
...
| `--pb2kslib-cache-workers` | `1..32` | `6` | Uses independent QEMU/DSP processes to generate disjoint PCM ranges and merge them before launch. `1` keeps generation in the game window. | `scripts/run-qemu.sh --dcs-engine pb2kslib-adsp --pb2kslib-cache-workers 8` |

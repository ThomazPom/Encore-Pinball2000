...
| `--pb2kslib-cache-workers` | `1..32` | `4` | Uses independent QEMU/DSP processes to generate disjoint PCM ranges and merge them before launch. `1` keeps generation in the game window. | `scripts/run-qemu.sh --dcs-engine pb2kslib-adsp --pb2kslib-cache-workers 8` |

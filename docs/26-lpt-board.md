...
| `scripts/run-qemu.sh --lpt-device disconnected` | Present an open, unconnected LPT bus to the ROM without board emulation. |
| `scripts/run-qemu.sh --lpt-device /dev/parport0` | Forward accesses to an explicitly selected Linux `ppdev` port. |

...
| Console scripting assertions | Add `@wait-for`, `@assert`, `@repeat`, `@screenshot` and audio-capture actions to `--script`. | Converts demonstrations into state-based regression tests instead of fixed-delay macros. |

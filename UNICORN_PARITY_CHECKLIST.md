...
|---|---|---|---|
| **Game speed rides raw host CPU speed → inconsistent across power states** — with no wall-clock pacing the guest ran full-speed = whatever the host delivered, so game speed swung wildly with the CPU governor: crawling on a throttled/battery CPU (vt-scale <1, ~14 MIPS) and racing **5-8x** on a boosted AC CPU (measured vt-scale 4.11x / 82 MIPS on this box under `--no-realtime`). Main (QEMU) paces to virtual/real time and feels "smooth vaseline butter". | Made main's wall-clock throttle (`--realtime`) the **default** (`main.c` init + `--no-realtime` opt-out): once per vblank cadence the cpu loop `nanosleep`s while guest virtual time (vticks) leads wall time at the 20 MHz model rate, so game speed is host-independent. `run-bundle-matrix.py` now passes `--no-realtime` to keep measuring full-speed boot. | _this session_ | A/B swe1 headless, identical flags: **default → vt-scale pinned 1.00x / host 20.0 MIPS** steady, delivered 50.1%, zero EIP-in-GDT/IStack; **`--no-realtime` → vt-scale 3.85–4.11x / 76–82 MIPS** (reproduces the AC "crazy" case). Both crash-free with the min_gap fix. |
